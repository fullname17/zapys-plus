...
class _ClientsScreenState extends ConsumerState<ClientsScreen> {
  ...
  /// ?sheet=client відкриває лист створення одразу — лист теж екран, який
  /// треба звіряти з макетом (той самий механізм, що й ?view= у календарі).
  void _maybeOpenBootSheet() {
    if (_sheetOpened || bootParam('sheet') != 'client') return;
    _sheetOpened = true;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) showCreateClientSheet(context);
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final loading =
        skeletonPreview() || (clientsAsync.isLoading && !clientsAsync.hasValue);
    final all = clientsAsync.value ?? const <Client>[];
